...
export async function POST(req: NextRequest) {
  try {
    ...
    const { sotaMarkdown, researchTopic, projectId } = await req.json();

    if (!sotaMarkdown || !researchTopic) {
      return NextResponse.json({ error: 'Missing sotaMarkdown or researchTopic' }, { status: 400 });
    }

    // Generate Gap and Novelty
    const gapMarkdown = await generateGapAndNovelty(sotaMarkdown, researchTopic);
