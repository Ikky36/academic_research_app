...
export async function generatePreResearchChatAction(messages: any[]) {
  try {
    const { callDeepSeekWithRetry } = await import('@/services/deepseek');
...
- Gunakan bahasa akademik Indonesia yang semi-formal, suportif, namun sangat analitis.\;
    ...
    // messages array already contains the user/assistant history.
    // we need to format it for callDeepSeekWithRetry
    // wait, callDeepSeekWithRetry takes (userPrompt: string, systemPrompt: string, mode, ...)
    // which only takes a single string. We should use a lower-level call or construct the prompt.
    // Wait, let's look at deepseek.ts to see if it supports full chat history.
  } catch (e: any) {
    return { error: e.message };
  }
}
